...
class Robohash(object):
    ...
    ALLOWED_CREATURE_SETS = {'robots': 1, 'zombies': 2, 'heads': 3}
    ...
    def __init__(self, **kwargs):
        ...
        self.creature_set = kwargs.get('set')
        ...
        self.gravatar_hashed = kwargs.get('use_gravatar', False)
    ...
    def __call__(self, text, **kwargs):
        ...
        bgset = kwargs.get('bgset', self.bgset)
        creature_set = kwargs.get('set', self.creature_set)
        ...
        if color:
            ...
            url += param if url.endswith('?') else '?{0}'.format(param)
        if creature_set in self.ALLOWED_CREATURE_SETS.items():
            param = 'set={0}'.format(self.ALLOWED_CREATURE_SETS[creature_set] if creature_set in self.ALLOWED_CREATURE_SETS else creature_set)
            url += param if url.endswith('?') else '?{0}'.format(param)
        return url
